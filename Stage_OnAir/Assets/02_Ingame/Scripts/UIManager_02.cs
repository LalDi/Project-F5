...
public class UIManager_02 : MonoBehaviour
{
    ...
    public ProgressDel TutorialProgress;
    ...
    public void Tutorial(int Sequence)
    {
        ...
        {
            case 0:
                TutorialMessage.transform.GetChild(0).GetComponent<Text>().text
                    = "지금부터 튜토리얼을 시작하겠습니다.";
                TutorialNum = 1;
                break;
            case 1:
            ...
            case 2:
                ...
                TutoParent = Text_Money.transform.parent.parent;
                TutoObjNum = Text_Money.transform.parent.GetSiblingIndex();

                //튜토리얼 배경 위로 오브젝트 가져오기
                Text_Money.transform.parent.SetParent(TutorialBG.transform);

                TutorialMessage.transform.localPosition = new Vector3(0, 570);
                TutorialMessage.transform.GetChild(0).GetComponent<Text>().text
            ...
            case 3:
                //저장해둔 위치로 다시 이동
                Text_Money.transform.parent.SetParent(TutoParent);
                Text_Money.transform.parent.SetSiblingIndex(TutoObjNum);

                TutoParent = Text_Month.transform.parent.parent;
                TutoObjNum = Text_Month.transform.parent.GetSiblingIndex();

                Text_Month.transform.parent.SetParent(TutorialBG.transform);
            ...
            case 4:
                ...
                break;
            case 5:
                Text_Month.transform.parent.SetParent(TutoParent);
                Text_Month.transform.parent.SetSiblingIndex(TutoObjNum);
                MonthorDate = true;

                TutoParent = Stat_UI.transform.parent;
                TutoObjNum = Stat_UI.transform.GetSiblingIndex();

                Stat_UI.transform.SetParent(TutorialBG.transform);

                TutorialMessage.transform.localPosition = new Vector3(0, 200);
                TutorialMessage.transform.GetChild(0).GetComponent<Text>().text
            ...
            case 11:
                ...
                break;
            case 12:
                Stat_UI.transform.SetParent(TutoParent);
                Stat_UI.transform.SetSiblingIndex(TutoObjNum);

                TutoParent = Forder_UI.transform.parent;
                TutoObjNum = Forder_UI.transform.GetSiblingIndex();

                Forder_UI.transform.SetParent(TutorialBG.transform);

                TutorialMessage.transform.localPosition = new Vector3(-100, 450);
                TutorialMessage.transform.GetChild(0).GetComponent<Text>().text 
                    = "오른쪽의 폴더를 열면 세가지의 버튼이 있습니다.";
                TutorialNum = 13;
                break;
            case 13:
                if (Forder_UI.transform.GetComponent<SettingMenu>().IsOpen == false)
                    Forder_UI.transform.GetChild(3).GetComponent<Button>().onClick.Invoke();
                for (int i = 0; i < 3; i++)
                    Forder_UI.transform.GetChild(i).GetComponent<Button>().enabled = false;
                ...
                break;
            case 14:
                if (Forder_UI.transform.GetComponent<SettingMenu>().IsOpen == true)
                    Forder_UI.transform.GetChild(3).GetComponent<Button>().onClick.Invoke();

                Forder_UI.transform.SetParent(TutoParent);
                Forder_UI.transform.SetSiblingIndex(TutoObjNum);

                TutoParent = Popup_Option.transform.parent;
                TutoObjNum = Popup_Option.transform.GetSiblingIndex();

                Popup_Option.transform.SetParent(TutorialBG.transform);
                Popup_Option.transform.SetSiblingIndex(0);

                Popup_On(0);

                TutorialMessage.transform.localPosition = new Vector3(0, -550);
                TutorialMessage.transform.GetChild(0).GetComponent<Text>().text 
                    = "설정에서는 음악 설정, 데이터 저장, 닉네임 변경 등의 기능이 있습니다.";
                TutorialNum = 15;
                break;
            case 15:
                Popup_Option.transform.SetParent(TutoParent);
                Popup_Option.transform.SetSiblingIndex(TutoObjNum);

                TutoParent = Popup_Rank.transform.parent;
                TutoObjNum = Popup_Rank.transform.GetSiblingIndex();

                Popup_Rank.transform.SetParent(TutorialBG.transform);
                Popup_Rank.transform.SetSiblingIndex(0);
            ...
            case 16:
                ...
                Popup_Rank.transform.SetParent(TutoParent);
                Popup_Rank.transform.SetSiblingIndex(TutoObjNum);

                TutoParent = Forder_UI.transform.GetChild(2).parent;
                TutoObjNum = Forder_UI.transform.GetChild(2).GetSiblingIndex();

                Forder_UI.transform.GetChild(2).SetParent(TutorialBG.transform);
            ...
            case 17:
                ...
                break;
            case 18:
                TutorialBG.transform.GetChild(1).SetParent(TutoParent);
                Forder_UI.transform.Find("Illust BT").SetSiblingIndex(TutoObjNum);

                if (Forder_UI.transform.GetComponent<SettingMenu>().IsOpen == true)
                    Forder_UI.transform.GetChild(3).GetComponent<Button>().onClick.Invoke();

                TutoParent = Bottom_UI.GetChild(1).parent;
                TutoObjNum = Bottom_UI.GetChild(1).GetSiblingIndex();

                Bottom_UI.GetChild(1).GetComponent<Button>().enabled = false;
                Bottom_UI.GetChild(1).SetParent(TutorialBG.transform);

                TutorialMessage.transform.localPosition = new Vector3(0, -450);
                TutorialMessage.transform.GetChild(0).GetComponent<Text>().text
            ...
            case 19:
                TutorialBG.transform.GetChild(1).SetParent(TutoParent);
                Bottom_UI.Find("Shop BT").SetSiblingIndex(TutoObjNum);
                Bottom_UI.Find("Shop BT").GetComponent<Button>().enabled = true;

                TutoParent = Popup_Shop.transform.parent;
                TutoObjNum = Popup_Shop.transform.GetSiblingIndex();

                Popup_Shop.transform.SetParent(TutorialBG.transform);
                ...
                break;
            case 20:
                Popup_Shop.transform.SetParent(TutoParent);
                Popup_Shop.transform.SetSiblingIndex(TutoObjNum);

                TutoParent = Bottom_UI.GetChild(4).parent;
                TutoObjNum = Bottom_UI.GetChild(4).GetSiblingIndex();

                Bottom_UI.GetChild(4).GetComponent<Button>().enabled = false;
                Bottom_UI.GetChild(4).SetParent(TutorialBG.transform);
            ...
            case 21:
                TutorialBG.transform.GetChild(1).SetParent(TutoParent);
                Bottom_UI.Find("Steff BT").SetSiblingIndex(TutoObjNum);
                Bottom_UI.Find("Steff BT").GetComponent<Button>().enabled = true;

                TutoParent = Popup_Staff.transform.parent;
                TutoObjNum = Popup_Staff.transform.GetSiblingIndex();

                Popup_Staff.transform.SetParent(TutorialBG.transform);

                Popup_On(12);
                Transform ScrollObj_1 = Popup_Staff.transform.Find("Scroll Rect Mask").GetChild(0);
                ...
                break;
            case 22:
                Popup_Staff.transform.SetParent(TutoParent);
                Popup_Staff.transform.SetSiblingIndex(TutoObjNum);

                Transform ScrollObj_2 = Popup_Staff.transform.Find("Scroll Rect Mask").GetChild(0);
                ...
                TutoParent = Bottom_UI.GetChild(2).parent;
                TutoObjNum = Bottom_UI.GetChild(2).GetSiblingIndex();

                Bottom_UI.GetChild(2).GetComponent<Button>().enabled = false;
                Bottom_UI.GetChild(2).SetParent(TutorialBG.transform);
            ...
            case 23:
                ...
                break;
            case 24:
                TutorialBG.transform.Find("Progress BT").SetParent(TutoParent);
                Bottom_UI.Find("Progress BT").SetSiblingIndex(TutoObjNum);
                Bottom_UI.GetChild(2).GetComponent<Button>().enabled = true;
                
                TutoParent = Bottom_UI.GetChild(3).parent;
                TutoObjNum = Bottom_UI.GetChild(3).GetSiblingIndex();

                Bottom_UI.GetChild(3).SetParent(TutorialBG.transform);

                TutorialMessage.transform.localPosition = new Vector3(0, -350);
                TutorialMessage.transform.GetChild(0).GetComponent<Text>().text
            ...
            case 25:
                ...
                break;
            case 26:
                TutorialBG.transform.Find("Progress Gauge_BG").SetParent(TutoParent);
                Bottom_UI.Find("Progress Gauge_BG").SetSiblingIndex(TutoObjNum);
        ...
        TutorialBG.GetComponent<Button>().onClick.RemoveAllListeners();
        TutorialBG.GetComponent<Button>().onClick.AddListener(delegate { TutorialProgress(); });
    }
